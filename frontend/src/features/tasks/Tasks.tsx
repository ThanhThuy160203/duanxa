...
const Tasks = () => {
  ...
  const handleCreateTask = () => {
    ...
    void withAction(
      ...
      "Đã tạo nhiệm vụ mới trên Firebase"
    );
  };
  ...
  const handleSeedData = () => {
    void withAction(() => seedTasksIfEmpty(), "Đã khởi tạo dữ liệu mẫu nếu Firebase đang rỗng");
  };
    ...
    <Stack spacing={3}>
      <Box>
        ...
        <Typography color="text.secondary">
          Dữ liệu realtime từ Firebase: giao nhiệm vụ, phản hồi hoàn thành, đánh giá và hủy theo đúng quyền vai trò.
        </Typography>
      </Box>
        ...
        {summary.map((item) => (
          <Card key={item.label} sx={{ minWidth: 210, flex: "1 1 210px", borderRadius: 3 }}>
            <CardContent>
              ...
              <Chip size="small" color={item.color} variant="outlined" label={loading ? "Đang đồng bộ" : "Realtime"} sx={{ mt: 1 }} />
            </CardContent>
          </Card>
        ))}
      </Stack>
        ...
        <CardContent>
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} md={6}>
              <TextField fullWidth label="Tiêu đề nhiệm vụ mới" value={newTaskTitle} onChange={(event) => setNewTaskTitle(event.target.value)} />
            </Grid>
            ...
            <Grid item xs={12} md={4}>
              <TextField fullWidth label="Người nhận" value={assignee} onChange={(event) => setAssignee(event.target.value)} />
            </Grid>
            <Grid item xs={12} md={4}>
              <FormControl fullWidth>
            ...
            <Grid item xs={12} md={4}>
              <TextField fullWidth label="Phòng ban" value={department} onChange={(event) => setDepartment(event.target.value)} />
            </Grid>
          ...
          <Stack direction="row" spacing={1.5} mt={2} flexWrap="wrap" useFlexGap>
            ...
            {capability?.canAssignTask && (
              <Button variant="outlined" onClick={handleAssignTask} disabled={saving || !selectedTaskId}>
                Giao nhiệm vụ
              </Button>
            )}
            {capability?.canReassignTask && (
              <Button variant="outlined" onClick={handleReassignTask} disabled={saving || !selectedTaskId}>
                Giao lại
              </Button>
            )}
            {capability?.canProvideFeedback && (
              <Button variant="outlined" onClick={handleSubmitFeedback} disabled={saving || !selectedTaskId || !feedback.trim()}>
                Phản hồi xác nhận hoàn thành
              </Button>
            )}
            {capability?.canEvaluateTask && (
              <>
                <Button variant="outlined" color="success" onClick={() => handleEvaluate(true)} disabled={saving || !selectedTaskId || !evaluationComment.trim()}>
                  Duyệt hoàn thành
                </Button>
                <Button variant="outlined" color="warning" onClick={() => handleEvaluate(false)} disabled={saving || !selectedTaskId || !evaluationComment.trim()}>
                  Yêu cầu làm lại
                </Button>
              </>
            )}
            {capability?.canCancelTask && (
              <Button variant="outlined" color="error" onClick={handleCancelTask} disabled={saving || !selectedTaskId}>
                Hủy nhiệm vụ
              </Button>
            )}
          </Stack>
